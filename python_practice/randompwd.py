...
def generate(n):
    ...
    then convert that string into list then uses random.shuffle to shuffle that list initiale another list in which we store our password
    then iterate a loop to password length so it can save characters through random.choice in pw_list
    shuffle pw_list for more randomness converts pw_list into string password. 
    """
